...
def main():
    ...
    def train_xgboost(train_data, future_days=60):
        try:
            ...
            # For future prediction, we'd need to create future features
            ...
            for _ in range(future_days):
                ...
                last_features[0][1] = pred * 1.01  # Update High (simplified)
                last_features[0][2] = pred * 0.99  # Update Low (simplified)
    ...
    if 'future_predictions' in st.session_state:
        ...
        with tab2:
            ...
            # Historical data
            fig.add_trace(go.Scatter(
                x=df_enhanced.index[-100:],  # Last 100 days
            ...
            fig.update_layout(
                title=f'Apple Stock Price Predictions (Next {prediction_days} Days)',
                xaxis_title='Date',
                yaxis_title='Price ($)',
                template='plotly_white',
                height=500
        ...
        # Welcome message
        st.info("""
        🎯 **Welcome to Apple Stock Price Predictor!**
        ...
        This app uses multiple machine learning models to predict Apple stock prices 
        ...
        with st.expander("📁 Dataset Overview"):
            ...
            with col1:
                ...
                st.write(f"Features: {len(df_enhanced.columns)}")
            
            with col2:
                st.write("**Recent Prices**")
                st.dataframe(df.tail()[['Open', 'High', 'Low', 'Close', 'Volume']].style.format("{:.2f}"))
